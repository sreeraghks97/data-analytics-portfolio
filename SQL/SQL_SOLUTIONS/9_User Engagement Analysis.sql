...
SELECT Posts.post_id, Posts.post_content , count(CASE WHEN UserReactions.reaction_type='like' THEN 1 END),count(CASE WHEN UserReactions.reaction_type='comment' THEN 1 END),count(CASE WHEN UserReactions.reaction_type='share' THEN 1 END) FROM Posts FULL OUTER JOIN UserReactions ON Posts.post_id=UserReactions.post_id WHERE Posts.post_id=2 GROUP BY Posts.post_id, Posts.post_content;

SELECT DATE(reaction_date) AS reaction_date ,COUNT(DISTINCT(user_id)) AS distinct_users,COUNT(*) AS total_reactions , ROUND(COUNT(*)::DECIMAL / COUNT(DISTINCT user_id), 4) AS avg_reactions_per_user FROM UserReactions GROUP BY DATE(reaction_date);

SELECT Posts.post_id,Posts.post_content, count(*) FROM Posts FULL OUTER JOIN UserReactions ON Posts.post_id=UserReactions.post_id GROUP BY Posts.post_id, Posts.post_content ORDER BY Posts.post_id LIMIT 3;
